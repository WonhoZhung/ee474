...
class ResNet(nn.Module):
    ...
    def cam(self, x, idx):
        ...
        params = params[-2]
        b, nc, h, w = out.shape
        gap = torch.zeros(b, 1, params.shape[1]).cuda()
        for i in range(b):
            gap[i][0] = params[idx[i].cpu().numpy()]
        out = torch.bmm(gap, out.reshape((b, nc, h * w)))
        # out = gap.dot(out.reshape((b, nc, h * w)))
        return out
...
class net(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10, num_layers=17, num_features=64):
